...
| X2a | Config validation over the nine fields of ADR 0011, progress log, `dcs.log` lines. Reading and writing the config file belong to X13, which owns it | Every bad config field produces one log line and its default in place; a missing `output_dir` produces one line and no run (ADR 0012) | X1 |
...
| X13 | Control window (ADR 0011), and the only surface the config has: `output_dir` and `omit_sea_tiles`, an advanced section holding the six others, one line per validation problem against the field it belongs to, Start (writes `Config/DcsTerrainExtract.lua`, then leaves the stopped state) and Stop (halts and saves the manifest, so the run resumes). Never closable, shown only when `enabled` is true. Reads the config file at load to fill the controls. Opens with a spike: whether `CheckBox` and `EditBox` construct from the hook state, whether a `Window` can refuse to close, and whether the Mission Editor can be told from the main menu and from a running mission. Adds a stopped state to X4's machine, since the run now begins waiting for Start | The window appears with the Mission Editor open, a value edited in it survives a DCS restart, Start begins a run and Stop leaves one that resumes; forcing every widget call to fail leaves the run going and the extract identical | X2a |
...
| C3 | `extract`: manifest and journal parse, validation, tile reader; `dcsterrain check-extract`. The manifest's completeness is one `complete` boolean (ADR 0013) | Accepts the synthetic extract; rejects each corrupted variant (wrong size, missing tile, extra file, bad min/max, unknown version, journal ahead of manifest, `complete` claimed with tiles the grid implies still missing) | F1, C1 |
