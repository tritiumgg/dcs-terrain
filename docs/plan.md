...
| X2b | `dcs_build` from `autoupdate.cfg`; `terrain_dir` derived by scanning `Mods/terrains/*/entry.lua` for the id `GetTerrainConfig` returns (ADR 0011); `terrain_fingerprint` with a pure Lua SHA-256 as a sliced step, tested against a known vector | The fingerprint of a known file matches `sha256sum` of its first 1 MiB, and the directory is found for a theatre whose id is not its directory name | X2a |
